...
def test_class_pair_interactions():
    # Warfarin (anticoagulant) + Ibuprofen (nsaid) → high
    results = get_interactions([_med("warfarin")], _med("ibuprofen"))
    assert any(r.severity == "high" for r in results)

    # Codeine (opioid) + Diazepam (benzodiazepine) → high
    results = get_interactions([_med("diazepam")], _med("codeine"))
    assert any(r.severity == "high" for r in results)

    # Clarithromycin (cyp3a4_inhibitor) + Simvastatin (cyp3a4_substrate) → high
    results = get_interactions([_med("simvastatin")], _med("clarithromycin"))
    assert any(r.severity == "high" for r in results)

    # Lidocaine with epinephrine (sympathomimetic) + Carvedilol (nonselective_beta_blocker) → high
    results = get_interactions([_med("carvedilol")], _med("lidocaine with epinephrine"))
    assert any(r.severity == "high" for r in results)


def test_drug_condition_rules():
    # NSAID + renal impairment → high
    results = get_condition_interactions([_med("ibuprofen")], ["renal impairment"])
    assert any(r.severity == "high" for r in results)

    # Anticoagulant + liver disease → high
    results = get_condition_interactions([_med("warfarin")], ["liver disease"])
    assert any(r.severity == "high" for r in results)

    # Corticosteroid + diabetes → medium
    results = get_condition_interactions([_med("prednisone")], ["diabetes"])
...
def test_no_interaction_returns_empty():
    ...
    results = get_interactions([_med("metformin")], _med("omeprazole"))
    assert results == []

    # Unknown drug returns empty
    results = get_interactions([_med("aspirin")], _med("somefakedrug"))
    assert results == []

    # No conditions → empty
    results = get_condition_interactions([_med("warfarin")], [])
    assert results == []
